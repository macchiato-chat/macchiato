...
import { Projects, memHash } from "../src/openclaw/projects";

/** #227:register/mem_read/mem_write/registry + 安全紀律 + 回合末惰性版本化。 */

function setup() {
  const dir = mkdtempSync(join(tmpdir(), "openclaw-proj-"));
  process.env.MACCHIATO_OPENCLAW_PROJECTS = join(dir, "registry.json");
  const sent: any[] = [];
  const handlers: any[] = [];
  const linkb: any = {
    agentLinkId: "al1",
...
let workdir: string;
beforeEach(() => {
  workdir = mkdtempSync(join(tmpdir(), "openclaw-proj-ws-"));
});
...
describe("#227 mem 讀寫 + 安全紀律", () => {
  ...
  it("registry 對賬:全量替換;重啟後從本地文件恢復", () => {
    const { op } = setup();
    const dir2 = mkdtempSync(join(tmpdir(), "openclaw-proj-ws2-"));

...
function setup() {
  ...
  process.env.MACCHIATO_CC_PROJECTS = join(dir, "registry.json");
  const sent: any[] = [];
  const handlers: any[] = [];
  const linkb: any = {
    agentLinkId: "al1",
...
let workdir: string;
beforeEach(() => {
  workdir = mkdtempSync(join(tmpdir(), "cc-proj-ws-"));
});
...
describe("#227 mem 讀寫 + 安全紀律", () => {
  ...
  it("registry 對賬:全量替換;重啟後從本地文件恢復", () => {
    const { op } = setup();
    const dir2 = mkdtempSync(join(tmpdir(), "cc-proj-ws2-"));
...
describe("#378 符號連結:安全解析 + 包含性檢查(項目內照用、項目外拒)", () => {
  ...
  let secret: string;
  beforeEach(() => {
    secretDir = mkdtempSync(join(tmpdir(), "cc-secret-"));
  ...
  it("備案目錄事後被換成指向他處的 symlink → mem 操作拒(dev/ino 身份重驗)", () => {
    ...
    const other = mkdtempSync(join(tmpdir(), "cc-other-"));
...
describe("#498/F-14 ~/ 展開：server 形路徑作註冊表鍵、fs 走 $HOME", () => {
  it("register path=~/rel → 寫入 $HOME/rel；mem_read 仍用 ~/rel 鍵命中", () => {
    const fakeHome = mkdtempSync(join(tmpdir(), "cc-home-"));
    ...
    try {
      ...
      expect(existsSync(join(abs, "CLAUDE.md"))).toBe(true); // 墊片落在展開後目錄
      // 註冊表鍵是 server 形（含 ~/），不是展開後絕對路徑
      expect(op({ reqId: 2, op: "mem_read", path: serverPath }).agentsMd).toBe("# tilde-mem");
      expect(op({ reqId: 3, op: "mem_read", path: abs }).ok).toBe(false); // 絕對鍵未備案

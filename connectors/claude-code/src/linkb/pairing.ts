...
function attempt(serverUrl: string, webUrl: string, label: string, fresh: boolean): Promise<Creds> {
  return new Promise((resolve, reject) => {
    ...
    let settled = false;
    const sendPair = (): void => ws.send(JSON.stringify({ t: "pair_request", proto: LINK_B_PROTO, label, kind: "claude-code", ...(process.env.MACCHIATO_PAIR_BATCH ? { batch: process.env.MACCHIATO_PAIR_BATCH } : {}) }));

...
export function e2eControlStorePath(): string {
  const keyStorePath =
    process.env.MACCHIATO_CODEX_E2E_STORE ||
    join(homedir(), ".macchiato/codex-e2e.json");
  return (
    process.env.MACCHIATO_CODEX_E2E_CONTROL_STORE ||
...
export class E2EControlVerifier {
  ...
  private acquireReplayLock(): () => void {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      // lockDir 已原子移走；下一轮重新 mkdir。若另一进程抢先拿到，只会 fail closed。
    }
    throw new E2EControlError("failed to acquire E2E control replay lock");
  }

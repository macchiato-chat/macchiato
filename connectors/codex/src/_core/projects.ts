...
import { type ProjectsKind, projectsRegPath } from "./identity";
import { type DirIdentity, dirIdentity, namedFileSize, namedKind, readNamedFile, writeNamedFileAtomic } from "./fs/safe-fs";

function regFile(kind: ProjectsKind): string {
  return projectsRegPath(kind);
}
...
export class Projects {
  ...
  constructor(
    ...
    private readonly kind: ProjectsKind,
  ) {
    this.load();
  }
  ...
  private load(): void {
    try {
      const p = JSON.parse(readFileSync(regFile(this.kind), "utf8"));
  ...
  private save(): void {
    try {
      mkdirSync(dirname(regFile(this.kind)), { recursive: true });
      const tmp = `${regFile(this.kind)}.tmp`;
      writeFileSync(tmp, JSON.stringify({ v: 1, paths: [...this.reg.keys()] }));
      renameSync(tmp, regFile(this.kind));
    } catch (e) {

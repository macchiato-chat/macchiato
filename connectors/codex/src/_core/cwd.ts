/**
 * #392(claude-code) / #377(codex) 會話工作目錄(cwd)的校驗。兩家邏輯同構,只有 env 名不同
 * (見 identity.workdirEnvName / allowedRootsEnvName);#572 抽進 connector-core 單源。
 *
 * 背景/威脅:server 下發的 `session.create.cwd` 舊實現只 `trim()` 就透傳給 `query({ cwd })` /
 * `codex exec -C`,**零校驗**(無 realpath / 存在性 / 目錄身份 / allowlist)。在「server 不可信」
 * 的 E2E 威脅模型下這是本地擴權面:CC 側配合 acceptEdits 檔(編輯類自動批)可靜默改用戶文件;
 * Codex 側默認 workspace-write 沙箱的可寫範圍**隨 cwd 走**,未校驗的 cwd 直接等於未校驗的可寫範圍。
 *
 * 邊界(2026-07-25 人類 review 定調:安全收益要拿,但**不動用戶的路徑**):
 *  - **默認工作目錄仍是 HOME**(`<KIND>_WORKDIR || homedir()`)。中途試過改成專用工作區,產品代價
 ...
 *  - **可選 allowlist**(`<KIND>_ALLOWED_ROOTS`,冒號分隔)——opt-in,預設不啟用,且**只約束顯式
 *    下發的 cwd**。三態:
 ...
 * `<KIND>_WORKDIR` 指一個專用目錄——保持本機操作者說了算,連接器不替用戶改路徑。
 */
...
/** 本机默认工作目录：kind 对应 WORKDIR env 覆盖，否则 HOME。纯算路径，不碰 fs。 */
export function defaultWorkDir(kind: CwdKind): string {
  return process.env[workdirEnvName(kind)] || homedir();
}
...
/**
 * `<KIND>_ALLOWED_ROOTS`(冒號分隔)→ realpath 後的允許根。**只用於顯式下發的 cwd。**
 *  - **未設 / 只有空白** → `null` = 不限根(保 #227「自由文件夾」語義);
 *  - **設了但全部根不可解析**(盤沒掛 / 路徑打錯)→ `[]` = 顯式 cwd 一律拒。
 *    絕不能在這種情況回 null——配置壞掉退化成「完全不限根」是 fail-open,方向反了。
 */
export function allowedRoots(kind: CwdKind): string[] | null {
  const raw = process.env[allowedRootsEnvName(kind)]?.trim();
  if (!raw) return null;
  const roots: string[] = [];
  let declared = 0;
...
export function resolveCwd(kind: CwdKind, raw: string | undefined): CwdResolution {
  ...
  if (roots !== null && !roots.some((r) => within(r, canon))) {
    const envName = allowedRootsEnvName(kind);
    const reason = roots.length
      ? `工作目錄不在允許根內(${envName}):${canon}`

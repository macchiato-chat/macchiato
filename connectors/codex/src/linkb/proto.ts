...
/**
 * #231 一個可選 model(連接器動態上報,喂 client 的 model chip + effort chip;絕不硬編碼別名)。
 * CC = supportedModels()(ModelInfo);Codex = model/list(Model)。統一形狀:
 */
export interface ModelOption {
  /** 傳給 session.model 的值(CC 別名 opus/sonnet/fable… | codex model id)。 */
  id: string;
  label: string;
  description?: string;
  /** 該 model 可用的 effort 檔位(空/缺 = 不支持 effort,client 不顯 effort chip)。 */
  effortLevels?: string[];
  /** 默認 effort(codex 上游真值;CC 自 #553 起由產品定為 high,連接器真下發)。
   *  client 據此在 session.effort 空時顯示具體檔位(去掉「Default」,所見即所得)。 */
  defaultEffort?: string;
  /**
   * #542 本行 id 解析到的 canonical wire model id(SDK ModelInfo.resolvedModel;codex 無別名不帶)。
   * client 匹配持久化 model 串的兜底層:id 精確 → resolvedId → label 忽略大小寫。持久化串與
   * 上報 id 對不上時(硬編碼回退清單存過 "fable" 而清單行是 "fable[1m]"),chip 才不顯原始串。
   */
  resolvedId?: string;
  /**
   * #553 連接器在 session.model 空時實際運行的那一行(CC = default 行 resolvedModel 對回的別名行;
   * codex = model/list 的 isDefault)。client 據此:菜單去掉「Default」項、空 model 顯本行 label。
   * 全清單無此標(舊連接器/對不上)→ client 保留 Default 項(誠實回退)。
   */
  isDefault?: boolean;
}

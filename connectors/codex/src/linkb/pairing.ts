...
function showCode(code: string, webUrl: string, fresh: boolean): void {
  ...
  const who = process.env.MACCHIATO_PAIR_GROUP || "Codex";
...
function attempt(serverUrl: string, webUrl: string, label: string, fresh: boolean): Promise<Creds> {
  return new Promise((resolve, reject) => {
    ...
    let settled = false;
    const sendPair = (): void => ws.send(JSON.stringify({ t: "pair_request", proto: LINK_B_PROTO, label, kind: "codex", ...(process.env.MACCHIATO_PAIR_BATCH ? { batch: process.env.MACCHIATO_PAIR_BATCH } : {}) }));
...
export async function runPairing(opts: PairOptions = {}): Promise<Creds> {
  ...
  const label = opts.label || process.env.MACCHIATO_LABEL || `Codex (${hostname()})`;

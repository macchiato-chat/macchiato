 ...
 * #377 會話工作目錄(cwd)的校驗。
 *
 * 背景/威脅:server 下發的 `session.create.cwd` 舊實現只 `trim()` 就透傳給 `codex exec -C`,**零校驗**
 * (無 realpath / 存在性 / 目錄身份 / allowlist)。在「server 不可信」的 E2E 威脅模型下這是本地擴權面:
 * 默認 workspace-write 沙箱的可寫範圍**隨 cwd 走**,故未校驗的 cwd 直接等於未校驗的可寫範圍。
 *
 * 邊界(2026-07-25 人類 review 定調:安全收益要拿,但**不動用戶的路徑**):
 *  - **默認工作目錄仍是 HOME**(`MACCHIATO_CODEX_WORKDIR || homedir()`)。中途試過改成專用工作區,產品代價
 ...
 *  - **可選 allowlist**(`MACCHIATO_CODEX_ALLOWED_ROOTS`,冒號分隔)——opt-in,預設不啟用,且**只約束顯式
 *    下發的 cwd**。三態:
 ...
 * `MACCHIATO_CODEX_WORKDIR` 指一個專用目錄——保持本機操作者說了算,連接器不替用戶改路徑。
 */
import { realpathSync, statSync } from "node:fs";
import { homedir } from "node:os";
import { join, sep } from "node:path";

/** 本機默認工作目錄:`MACCHIATO_CODEX_WORKDIR` 覆蓋,否則 HOME。純算路徑,不碰 fs。 */
...
/**
 * `MACCHIATO_CODEX_ALLOWED_ROOTS`(冒號分隔)→ realpath 後的允許根。**只用於顯式下發的 cwd。**
...
export function allowedRoots(): string[] | null {
  const raw = process.env.MACCHIATO_CODEX_ALLOWED_ROOTS?.trim();
  if (!raw) return null;
  const roots: string[] = [];
  let declared = 0;
...
export function resolveCwd(raw: string | undefined): CwdResolution {
  ...
  if (roots !== null && !roots.some((r) => within(r, canon))) {
    const reason = roots.length
      ? `工作目錄不在允許根內(MACCHIATO_CODEX_ALLOWED_ROOTS):${canon}`

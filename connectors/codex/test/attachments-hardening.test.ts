/**
 * #383 附件落盤硬化(Codex 對稱 CC):權限、Content-Length 早拒、配額、失敗 unlink、並發、GC lstat。
 */
...
afterEach(() => {
  _resetAttachInflightForTest();
  delete process.env.MACCHIATO_CODEX_ATTACH_QUOTA_BYTES;
  delete process.env.MACCHIATO_CODEX_ATTACH_MAX_INFLIGHT;
});
...
describe("#383 Codex attachments hardening", () => {
  it("落盤目錄 0700、文件 0600(非 Windows)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-att-perm-"));
  ...
  it("Content-Length 超 DOWNLOAD_MAX → 早拒且不留 partial", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-att-cl-"));
    process.env.MACCHIATO_CODEX_ATTACH_DIR = dir;
    const { port, close } = await listen((_q, res) => {
  ...
  it("全局配額已滿 → 拒絕", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-att-quota-"));
    process.env.MACCHIATO_CODEX_ATTACH_DIR = dir;
    process.env.MACCHIATO_CODEX_ATTACH_QUOTA_BYTES = "100";
  ...
  it("並發達上限 → 拒絕", async () => {
    process.env.MACCHIATO_CODEX_ATTACH_MAX_INFLIGHT = "1";
  ...
  it("GC:lstat 不跟隨 symlink;清過期 .part", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-att-gc2-"));
    const outsideDir = mkdtempSync(join(tmpdir(), "codex-att-out-"));
    process.env.MACCHIATO_CODEX_ATTACH_DIR = dir;

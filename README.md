...
> The one-liner is served by macchiato.chat and is verified end to end: a version-pinned bootstrap checked by SHA-256, which then verifies an Ed25519-signed release manifest and every file hash before anything runs. Installer flags pass straight through, e.g. `… | bash -s -- --agents=claude-code --no-mirror`.
>
...
> Without it, systemd stops your user's service manager seconds after you log out — killing the connector — so the agent would go offline when you close the terminal. If the installer says it couldn't enable it, run `sudo loginctl enable-linger $USER` once. Lingering also starts the connector at boot without a login.
</details>
